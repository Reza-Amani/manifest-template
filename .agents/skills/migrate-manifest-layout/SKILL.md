...
+ `.local_manifest/` + a gitignored `.agents/` shim so Cursor, VS Code Copilot,
and OpenCode auto-discover skills.
...
3. Preserve the target's own wording. Rewrite addresses and links; do not
   rephrase mandates, decisions, or procedures. In `solo` targets, generated
   and rewritten files must not introduce team-versus-local ownership language
   (no "shared" vs "personal", no "if the local manifest is present"). Both
   trees are just directories; both are tracked.
...
List unexpected files at `.team_manifest/` root (for example `todo.plan.md`).
Do not invent a `todo.plan.md` when the repo has none.
...
Create or rewrite these, using the **repo-type** templates in
...
- **Root `AGENTS.md`.** Keep the target's existing first-read file if it
  ...
  target's real mandates verbatim. In `solo`, use the solo template: both
  trees are tracked, no shared-versus-personal wording. If
  `.team_manifest/todo.plan.md` exists, mention it next to the backlog.
...
- **Split routing.** Take the old `routing.rule.md`. Do not replace the file
  ...
  add a row for `backlog.plan.md`, and a row for `todo.plan.md` if that file
  exists. Rows whose target now lives under `.local_manifest/` go to
  `.local_manifest/rules/routing.rule.md`. Extra sections the target already
  had stay. Create the local file if missing. Remove `operation-*` rows.
  Keep each row's wording. In `solo`, headings must not say "team" vs
  "personal"; they may name the directory path.
...
- **`.team_manifest/rules/manifest-structure.rule.md`** — add the Layout
  section for this repo type from the template near the top and fix its
  `.agents/` mentions. Do not rewrite the folder-by-folder text; a full
  refresh arrives later through `sync-manifest-template`.
...
- `team`: remind the user to ignore `scratch/` in the private repo. If the
  ...
  un-ignore block from [`reference.md`](reference.md#ignore-blocks) so agents
  can still see the gitignored local tree.
...
- `solo` generated files do not talk about shared vs personal ownership or
  "if local is present".
...
- `solo`: generated text does not contrast team vs local ownership; both
  trees are tracked; `reference/` and `skills/` live only under
  `.local_manifest/`.
- `.team_manifest/rules/routing.rule.md` and
  `.local_manifest/rules/routing.rule.md` exist. Each routes into its own
  tree (plus `backlog.plan.md`, and `todo.plan.md` when present, in the team
  file).
