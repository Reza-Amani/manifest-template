...
description: >-
  ...
  initial plan under `.local_manifest/plans/archive/` after `implement-plan` finishes
...
Move one completed task's planning artifacts from `.local_manifest/plans/` to
`.local_manifest/plans/archive/`. This is closeout after `implement-plan`, not a way to
...
4. Reject sources outside `.local_manifest/plans/` and existing destinations.
