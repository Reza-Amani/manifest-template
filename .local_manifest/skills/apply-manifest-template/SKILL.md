...
description: >-
  ...
  tailored two-directory manifest (or fixes an existing one) with real,
  repo-specific content instead of the template's example files.
...
4. The agent reads the template, studies the target repo, and writes a tailored
   manifest into the **target** repo's `.team_manifest/` and `.local_manifest/`
   — creating documents if the trees are empty, or upgrading them if content
   already exists.
...
- **Target repo (destination):** the other workspace folder. This is where the
  manifest is written.
- **Repo type:** `solo` or `team`. Ask the user which layout they chose during
  manual setup, then verify it (`.local_manifest` real directory = `solo`;
  symbolic link = `team`).
...
**Tree roots and the discovery shim are the user's job.** Before running, the
target must already have `.team_manifest/`, `.local_manifest/`, and the
gitignored `.agents/skills/` shim (`local` in `solo`; `team` and `local` in
`team`). If any of these is missing, stop and hand the user
[`user-setup.md`](user-setup.md); do not create roots or symlinks yourself.

## Where files go

This template is a **solo** repo: every skill and every `*.ref.md` lives under
`.local_manifest/`. Do **not** copy that placement blindly into the target.
Place each file by the **target's** repo type using the ownership table and
entry-point templates in
[`manifest-layout.md`](../../../.team_manifest/guides/manifest-layout.md).
Never copy this template's solo wording blindly into a team target.
...
```
Task progress:
- [ ] 1. Identify source (this template) and target repo, and repo type
- [ ] 2. Learn the template structure and conventions and agent rules/instructions
- [ ] 3. Detect whether the target already has manifest content
...
- [ ] 10. Replace template-domain examples with real content; use only a neutral, explicitly marked placeholder where an otherwise-empty directory needs guidance.
- [ ] 11. Merge the repo-type `.gitignore` block
...
and the repo type with the user if there is any ambiguity.
...
- [`../../../.team_manifest/rules/manifest-structure.rule.md`](../../../.team_manifest/rules/manifest-structure.rule.md)
  — folder purpose, naming suffixes, and placement rules. **This file is
  required in the target.**
- [`../../../.team_manifest/guides/manifest-layout.md`](../../../.team_manifest/guides/manifest-layout.md)
  — the canonical layouts, ownership table, entry-point templates, shim
  commands, and ignore rules. Copy this guide into the target.
- [`../../../.team_manifest/rules/routing.rule.md`](../../../.team_manifest/rules/routing.rule.md)
  and
  [`../../rules/routing.rule.md`](../../rules/routing.rule.md)
  — how the two routing maps are laid out.
- [`../../../.team_manifest/rules/terms.rule.md`](../../../.team_manifest/rules/terms.rule.md)
  — the terms format. Add only layout terms that apply to the target type.
- [`../../../AGENTS.md`](../../../AGENTS.md) — entry-point shape. Rewrite it
  with the matching template from `manifest-layout.md`.
...
Check the target repo for `AGENTS.md`, `.team_manifest/`, and
`.local_manifest/`.

- **Trees exist, little or no content:** you will create the documents (step 6,
  "Create" path).
...
Write into the **target** repo. Follow the placement table above and the
naming suffixes from `manifest-structure.rule.md`.

**Always produce (required core):**

- `AGENTS.md` at the target root — real, short project mandates and pointers to
  both routing files (and the unconditional read of `.local_manifest/AGENTS.md`).
  Use the solo or team wording from `manifest-layout.md`.
- `.team_manifest/rules/manifest-structure.rule.md` — copy the template's
  version.
- `.team_manifest/guides/manifest-layout.md` — copy the canonical guide.
- `.team_manifest/rules/routing.rule.md` — real routing tables for docs that
  live under `.team_manifest/`. Remove example rows. Add a backlog row.
- `.local_manifest/rules/routing.rule.md` — real routing tables for docs that
  live under `.local_manifest/`. Remove example rows.
- `.local_manifest/AGENTS.md` — create only if missing. Point at
  `rules/routing.rule.md` in the local tree.
- `.team_manifest/rules/terms.rule.md` — real terms from the target's
  vocabulary (step 4), plus the layout terms for the target type.

**Produce as the repo warrants (tailored, not example copies):**

- `.team_manifest/adr/` — start with the meta-ADR (record architecture decisions), then
  one ADR per real, already-settled decision from steps 4–5.
- `reference/` — copy the generic `planning-flow.ref.md` into
  `.local_manifest/reference/`, then add a `*.ref.md` per real subsystem worth
  documenting (local on `solo`; team tree only when everyone must know it). If
  you didn't find anything, leave a placeholder as an example for future
  reference, but mark it clearly as an example.
- `.team_manifest/guides/` — `setup.md` and `commands.md` filled with the
  target's real setup steps and commands; add `notes.md` with the user's
  temporary notes. Link setup to `manifest-layout.md` instead of duplicating
  layout and shim instructions.
- `.team_manifest/backlog.plan.md` — seeded from the target's real open work, or
  leave it shallow with just some brief general guides.
- Other `.local_manifest/plans/` content only when it comes from real target work:
...
- `.local_manifest/learning/README.md` — explains where `teach` skill session records
  belong and how they are named.
- `.local_manifest/findings/README.md` — explains where reproducible analysis and
  investigation records belong (perf digs, root-cause analyses) and how they are
  named.
- `.team_manifest/personas/` — only personas that fit the target (adapt `reviewer`;
...
- `.local_manifest/skills/` — include the generic planning workflow skills
  ...
  reviewer persona and routing files. Do not copy `apply-manifest-template`,
  `sync-manifest-template`, `migrate-manifest-layout`, or
  `create-github-repo` into the target; those remain template-side workspace
  tools.
...
- `.team_manifest/actions/verify-docs-in-sync.md`. Also, any other "action" or "prompt" that you find in the existing system prompts.

**Ignore files:** merge the target-type blocks from
[`manifest-layout.md`](../../../.team_manifest/guides/manifest-layout.md#ignore-rules).
Do not add old `.cursor/` / `.github/skills` mirror entries unless the target
still uses those mirrors.
...
- No "example subsystem", "example feature", "User Sync", or other
  template-domain content should remain in the target.
- Never copy `.local_manifest/plans/example-epic/`; create an epic directory only for
  real target work and use the target's own names and content.
- Delete example files you did not turn into real content. Where an empty
  directory needs guidance, use one short, neutral placeholder instead of a
  copied domain example.
- Keep `manifest-structure.rule.md` (generic and required) and the structural
  shape of both routing files, but their entries must reference real target docs.
...
- Root `AGENTS.md` points at both routing files. Each routing file points at
  docs that exist in its own tree (plus backlog in the team file).
- `solo` targets have no `.team_manifest/reference/` or `.team_manifest/skills/`.
- `team` targets have no duplicate skill folder names across the two trees.
...
[`../../../.team_manifest/guides/setup.md`](../../../.team_manifest/guides/setup.md),
and its canonical `manifest-layout.md`).
...
Target trees + shim present?
├─ No ................................. STOP: hand user-setup.md
└─ Yes
    Target has manifest content?
    ├─ No ............................. CREATE documents (step 6)
    └─ Yes ............................ UPGRADE in place; never overwrite ADRs
```

## Completion checks

- The target repo has at minimum: root `AGENTS.md`,
  `.team_manifest/rules/manifest-structure.rule.md`,
  `.team_manifest/guides/manifest-layout.md`, both routing files, and
  `.team_manifest/rules/terms.rule.md`.
- Every substantive file reflects the target's real business; no
  template-domain example content remains.
- Existing ADRs in the target were preserved (added to or flagged, never
  silently rewritten).
- Target `.gitignore` matches the repo type. `solo` tracks `.local_manifest/`.
- Files sit in the tree required by the target's repo type.
