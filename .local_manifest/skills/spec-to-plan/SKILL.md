...
name: spec-to-plan
description: >-
  Turns a spec into an implementation plan in Cursor Plan Mode format and saves
  it under the matching location in `.local_manifest/plans/`. Simple plans stay
  at the top level; epic plans stay beside their spec in the epic's
  subdirectory. It does not edit the spec or add acceptance criteria. Use when
  the user asks to "plan from this spec", "make a plan for the spec", "spec to
  plan", or wants a Cursor-style implementation plan before criteria and
  building.
---
...
**Cursor Plan Mode** does. Save the plan under `.local_manifest/plans/`. Do not change
...
- **Target repo (destination):** the repo the work is about. The plan goes there under `.local_manifest/plans/`.
...
2. Read relevant ADRs, references, and rules from the target's manifest trees when
...
```
# Simple work
.local_manifest/plans/<topic>.planning.md

# One part of an epic
.local_manifest/plans/<epic>/<part>.planning.md
```
