...
description: >-
  ...
  writes a spec file to the *target* repo's `.local_manifest/plans/` tree as the starting
...
glossary in a manifest repo is `.team_manifest/rules/terms.rule.md`; decisions live in
`.team_manifest/adr/*.adr.md`.
...
```
# Simple work
.local_manifest/plans/<topic>.spec.md

# One part of an epic
.local_manifest/plans/<epic>/<part>.spec.md
```
...
`.local_manifest/plans/` tree, and never in the template repo.
...
After the spec is written, check whether the session's decisions made any manifest files in the target repo stale. Follow the placement rules in the target repo's manifest structure (`.team_manifest/rules/manifest-structure.rule.md` if present).
...
- `.team_manifest/rules/terms.rule.md` — terms coined or sharpened during the session.
- `.team_manifest/adr/` — a decision from the session that is hard to reverse,
surprising without context, and the result of a real trade-off. If any of the
three is missing, skip the ADR.
- `reference/` — mechanics described in the session that contradict or
extend an existing `*.ref.md` (`.local_manifest/reference/` on solo; the tree
that already holds the file on team).
- `.team_manifest/guides/` — procedures or commands the session changed.
...
the user.
- The spec exists directly under `<target>/.local_manifest/plans/` for simple work or
