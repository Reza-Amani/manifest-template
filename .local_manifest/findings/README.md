...
repeat the query from scratch. They also preserve useful discoveries that were
not the result of a planned investigation.

Examples include data analyses, performance digs, and root-cause analyses.

> Findings are **not** decisions and **not** mechanics. A decision goes in
> [`adr/`](../../.team_manifest/adr/); how a subsystem works goes in
...
- **Sources & parameters:** which data sources, date range, filters, and query
  ...
  any credential, never secret values.
...
- Decisions → [`adr/`](../../.team_manifest/adr/).
