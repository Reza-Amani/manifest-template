...
`.team_manifest/` and `.local_manifest/`. Use it when adding, moving, renaming,
or reviewing manifest files, or when deciding whether information belongs in a
decision record (ADR), reference, rule, skill, guide, plan, action, persona, or
learning record.

> **This file is required.** It defines manifest document types and placement.
> The physical solo/team layouts and setup commands live in
> [`manifest-layout.md`](../guides/manifest-layout.md).

## Manifest Scope

The manifest is the union of `.team_manifest/` and `.local_manifest/`. Those
directories hold domain knowledge, decision records, operational notes, global
rules, and specialized task automation for AI agents and human contributors.
`.agents/` is only a gitignored discovery shim of skill symlinks. Canonical
content belongs in the two manifest directories, not in the shim.

## Layout: two directories and a shim

Use the canonical [`manifest-layout.md`](../guides/manifest-layout.md) guide.
It defines solo/team ownership, entry-point templates, the discovery shim,
ignore rules, and repo-type detection.
...
example a credential-handling rule. Name them `<category>.rule.md` and route
to them from `routing.rule.md`.
...
question, how it was answered, and the result, such as a data analysis,
performance dig, or root-cause analysis.
...
3. Read the routing file in this directory and
   `.local_manifest/rules/routing.rule.md` to find task-specific ADRs,
   references, skills, personas, plans, learning, findings, actions, or rules.
...
Skills are auto-discovered from the `.agents/skills/` shim. Rules, personas,
and other manifest docs are reached through root `AGENTS.md` and the routing
files. See [`../guides/manifest-layout.md`](../guides/manifest-layout.md) for
the shim and ignore rules.
