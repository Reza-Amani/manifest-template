...
change as complete. It detects whether a change has made any manifest doc
...
Quick gate: if the change does not touch anything named in `.team_manifest/`
or `.local_manifest/`, skip.
...
4. **Backlog sync** — if the change completes a backlog item, it is checked off
   in [`../backlog.plan.md`](../backlog.plan.md) and its YAML
